rule gatk_gathervcfs_chromosomewise:
    ...
    params:
        ' '.join(['-I '+os.path.join(os.path.abspath(output_folder),'GATK_GenotypeGVCFs_gz','{project_name}_{chromosome}.vcf.gz'.format(project_name=project_name, chromosome=chromosome)) for chromosome in chromosomes])
    output:
