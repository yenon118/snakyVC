...
rule sftp_gatk_haplotypecaller:
	...
	output:
		out_file = SFTP.remote(host+os.path.join(os.path.abspath(output_folder),'GATK_HaplotypeCaller_gvcf_gz','{sample}.g.vcf.gz')),
		out_tmp_dir = temp(directory(os.path.join(os.path.abspath(local_tmp_folder),'GATK_HaplotypeCaller_gvcf_gz','tmp','{sample}')))
	log:
		SFTP.remote(host+os.path.join(os.path.abspath(output_folder),'GATK_HaplotypeCaller_gvcf_gz_log','{sample}.log'))
	params:
		'-ERC GVCF'
	resources:
		memory = memory
