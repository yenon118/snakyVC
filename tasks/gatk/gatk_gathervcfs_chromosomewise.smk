rule gatk_gathervcfs_chromosomewise:
    input:
          ...
          in_file = expand(os.path.join(os.path.abspath(output_folder),'GATK_GenotypeGVCFs_gz','{project_name}_{{chromosome}}.vcf.gz'.format(project_name=project_name)), chromosome=chromosomes)
    params:
          ' '.join(['-I '+os.path.join(os.path.abspath(output_folder),'GATK_GenotypeGVCFs_gz'.format(chromosome=chromosome),'{project_name}_{chromosome}.vcf.gz'.format(project_name=project_name, chromosome=chromosome)) for chromosome in chromosomes])
    output:
